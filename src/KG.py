...
class KG(object):
    ...
    def load_freq(self, filename, splitter = ' ', line_end = '\n', freq_split = 30, r_ = '!!'):
        for line in open(filename, encoding='utf-8'):
            line = line.rstrip(line_end).split(splitter)
            self.ent_freq[line[0].replace(r_, ' ')] = int(line[1])
            self.ind_freq[self.index_ents[line[0].replace(r_, ' ')]] = int(line[1])
            if int(line[1]) not in self.freq_ent:
                self.freq_ent[int(line[1])] = [self.index_ents[line[0].replace(r_, ' ')]]
            else:
                self.freq_ent[int(line[1])].append(self.index_ents[line[0].replace(r_, ' ')])

            if int(line[1]) >= freq_split:
                self.high_freq.append(self.index_ents[line[0].replace(r_, ' ')])
            else:
                self.low_freq.append(self.index_ents[line[0].replace(r_, ' ')])

        self.low_high = int(len(self.low_freq)/len(self.high_freq))
